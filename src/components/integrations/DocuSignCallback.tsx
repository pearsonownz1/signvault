...
/**
 * This component handles the OAuth callback from DocuSign.
...
export default function DocuSignCallback() {
  ...
  useEffect(() => {
    ...
    if (error) {
      window.location.href = `/integrations/docusign-complete?error=${error}&message=${errorDescription || 'Unknown error'}`;
      return;
    }

    // If code or state is missing, redirect with an error
    if (!code || !state) {
      window.location.href = '/integrations/docusign-complete?error=invalid_request&message=Missing+required+parameters';
      return;
    }

    // Redirect to our backend API endpoint with the code and state
    window.location.href = `/api/docusign/callback?code=${code}&state=${state}`;
  }, [location]);

  return (
    <div className="flex flex-col items-center justify-center min-h-screen p-4">
      <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-primary mb-4"></div>
      <h1 className="text-2xl font-bold mb-2">Connecting to DocuSign</h1>
